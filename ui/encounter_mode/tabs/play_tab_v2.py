# ui/encounter_mode/tabs/play_tab_v2.py
            ...
            if phase == "idle":
                ...
                with c2:
                    if st.button("Use", key=f"gravestone_encounters_{ctx}_{i}", disabled=False, width="stretch"):
                        ...
                        for node in nodes[start_idx + 1 :]:
                            if node.get("kind") != "encounter":
                                continue
                            if node.get("choice_index") is None:
                                target = node
                                break
                ...
                with c3:
                    if st.button("Use", key=f"gravestone_treasure_{ctx}_{i}", width="stretch"):
                        row["phase"] = "done"
                        row["result"] = (
                            "Look at the top card of the treasure deck, then put it on the top or bottom of the deck."
                        )
                        st.rerun()
...
def render(settings: dict, campaign: bool = False) -> None:
    """Encounter Play tab (V2 / full-featured UI)."""
