...
# ---------------------------------------------------------------------------
# Campaign V1: optional invader configuration (mirrors Encounter Mode behavior)
# ---------------------------------------------------------------------------
...
def _persist_v1_frozen_update(*, campaign: Dict[str, Any], current_node: Dict[str, Any], frozen: Dict[str, Any]) -> None:
    """Persist mutated frozen encounter data into Campaign V1 session state and rerun."""
    current_node["frozen"] = frozen
    current_node["revealed"] = True

    # Update the canonical V1 state snapshot if present.
    v1_state = st.session_state.get("campaign_v1_state")
    if isinstance(v1_state, dict):
        v1_campaign = v1_state.get("campaign")
        if isinstance(v1_campaign, dict):
            v1_state["campaign"] = v1_campaign
        else:
            v1_state["campaign"] = campaign
        st.session_state["campaign_v1_state"] = v1_state
    else:
        st.session_state["campaign_v1_state"] = {"campaign": campaign}

    st.rerun()


def _render_v1_invader_setup_controls(*, campaign: Dict[str, Any], current_node: Dict[str, Any], frozen: Dict[str, Any]) -> None:
    """Invader add/remove UI for the current Campaign V1 encounter.
    ...
    with st.expander("Invaders for this encounter", expanded=False):
        ...
        # Locked invaders from enemies only
        ...
        # Added invaders are stored on frozen['invaders'] but exclude locked
            ...
            for idx, name in enumerate(added_names):
                ...
                with c2:
                    if st.button("❌", key=f"{key_prefix}_remove_{idx}", width="stretch"):
                        ...
                        _persist_v1_frozen_update(campaign=campaign, current_node=current_node, frozen=frozen)
    ...
    if not isinstance(campaign, dict):
        ...
        if fallback is None:
            try:
                ...
                disk_f = read_last_frozen("V1")
                if isinstance(disk_f, dict):
                    logging.getLogger("dsbg").info("Loaded campaign_v1_last_frozen from disk")
                    fallback = disk_f
            except Exception:
                logging.getLogger("dsbg").exception("Failed to read campaign_v1_last_frozen from disk")
...
def _render_v1_current_panel(
    ...
    if kind == "encounter":
        ...
        if isinstance(frozen, dict):
            _render_v1_invader_setup_controls(
                campaign=campaign,
